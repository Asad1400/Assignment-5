...
let radius = 5;
const PI = Math.PI;
let calculateArea = (radius) => PI * Math.pow(radius, 2);
...
const temperatures = [20, 25, 35, 12, 10];
...
let words = ["asad", "although", "mango", "apple", "run", "sad", "happy", "animal"];
...
let fruits = ["apple", "banana", "mango", "watermelon", "pinapple"];
...
let newNumbers = numbers2.map(x => x * x);
...
const removeFalsey = (array) => array.filter(Boolean); // The Boolean function is used as a callback to filter out falsey values.
...
const checkElement = (array, element) => array.indexOf(element);
...
const numbers4 = [10, 5, 20, 20, 4, 5, 2, 25, 1];
...
const calculateProduct = (array) => array.reduce((a, b) => a * b, 1);
...
const array7 = ["asad", "fahad", "ali", "going", "went", "moving"];
const filterByLength = (array, n) => array.slice(n);
...
const countOccurrences = (array, element) => array.filter(x => x === element).length;
...
const names = ["Asad", "Fahad", "Abdullah", "Saad", "Ahmad"];
const forrmatNames = (array) => {
    if (array.length === 1) {
        return array[0];
    }
...
const fahrenheitTemperatures = [32, 68, 104, 140, 176];
const temperatureConverter = (array) => array.map(x => (x - 32) * 5 / 9);
const celsiusTemperatures = temperatureConverter(fahrenheitTemperatures);
...
let temp = 0;
const swapElements = (array, firstIndex, secondIndex) => {
...
const characterCounter = (message, character) => {
...
const completedTask = (array) => array.filter(x => x["completed"] === false);
...
const numbers6 = [5, 3, 8, 1, 2, 9, 4, 7, 6];
